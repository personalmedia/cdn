...
// HandlePDFMarkdown extracts plain text from a PDF and returns it as a Markdown file.
func HandlePDFMarkdown(c *gin.Context, req *ActionRequest) {
	...
	cacheFile := cache.CacheFileForDerived(req.Action, req.RelPath, ".md")
	...
	result, err, _ := cache.SF.Do(cacheFile, func() (interface{}, error) {
		...
		baseName := strings.TrimSuffix(filepath.Base(req.SourceFile), filepath.Ext(req.SourceFile))
		...
		if err := os.WriteFile(cacheFile, []byte(mdContent), 0o644); err != nil {
			return nil, err
		}
